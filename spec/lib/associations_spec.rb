...
describe "Associations" do
  ...
  context 'Defining and Querying Associations' do
    ...
    it 'finds associations defined on its parent' do
      expect(
        AssociationResource.association_builders[:has_many_objects]
      ).to be_present

      expect(
        AssociationChildResource.association_builders[:has_many_objects]
      ).to_not be_present
    end

    it 'does not find associations defined on the child' do
      expect(
        AssociationResource.association_builders[:belongs_to_object]
      ).to_not be_present
    end

    context '.lookup_association' do

      it 'finds anything defined on the class or its parents' do
        expect(
          AssociationGrandChildResource.lookup_association(
            :has_many_objects
          )
        ).to be_instance_of(ApiResource::AssociationBuilder::HasManyBuilder)

        expect(
          AssociationGrandChildResource.lookup_association(
            :has_one_object
          )
        ).to be_instance_of(ApiResource::AssociationBuilder::HasOneBuilder)
      end

      it 'does not find anything defined on a subclass' do
        expect(
          AssociationChildResource.lookup_association(
            :has_one_object
          )
        ).to be_nil
      end

      it 'finds the first builder defined in a superclass with shadowing' do
        expect(
          AssociationGrandChildResource.lookup_association(
            :to_override
          )
        ).to be_instance_of(
          ApiResource::AssociationBuilder::BelongsToBuilder
        )
      end

      it 'shadows an immediate superclass' do
        expect(
          AssociationChildResource.lookup_association(
            :to_override
          )
      ...
      it 'does not override the superclass when shadowing' do
        expect(
          AssociationResource.lookup_association(
            :to_override
          )
        ).to be_instance_of(
          ApiResource::AssociationBuilder::HasOneBuilder
        )
      end

    end

    # These are all basically the same method so just test has_one
    context '.has_one?' do

      it 'returns true if the association is defined on this class' do
        expect(
          AssociationGrandChildResource.has_one?(:has_one_object)
        ).to be_true
      end

      it 'returns true if the association is define on a superclass' do
        expect(
          AssociationGrandChildResource.has_one?(:superclass_has_one)
        ).to be_true
      end

      it 'returns false if the association is shadowed' do
        expect(
          AssociationGrandChildResource.has_one?(:to_override)
        ).to be_false
      end

      it 'returns false if the association is defined on a subclass' do
        expect(
          AssociationResource.has_one?(:has_one_object)
        ).to be_false
      end

      it 'returns false if the association does not exist' do
        expect(
          AssociationGrandChildResource.has_one?(:fake_association)
        ).to be_false
      end

      it 'returns false if it is not a has_one association' do
        expect(
          AssociationGrandChildResource.has_one?(:has_many_objects)
        ).to be_false
      end

    end

    context '.has_one_class_name' do

      it 'finds a class name for an association on this class' do
        expect(
          AssociationGrandChildResource.has_one_class_name(:has_one_object)
        ).to eql('HasOneObject')
      end

      it 'finds a class name for an association on the superclass' do
        expect(
          AssociationGrandChildResource.has_one_class_name(
            :superclass_has_one
          )
        ).to eql('SuperclassHasOne')
      end

      it 'does not find a class for a shadowed association' do
        expect(
          AssociationGrandChildResource.has_one_class_name(:to_override)
        ).to be_nil
      end

      it 'does not find it if the association is defined on a subclass' do
        expect(
          AssociationResource.has_one_class_name(:has_one_object)
        ).to be_nil
      end

      it 'does not find a class for an association that does not exist' do
        expect(
          AssociationGrandChildResource.has_one_class_name(:fake_association)
        ).to be_nil
      end

      it 'does not find a class for a non has_one association' do
        expect(
          AssociationGrandChildResource.has_one_class_name(:has_many_objects)
        ).to be_nil
      end
    ...
    context '.association?' do
      ...
      it 'returns true if the association exists' do
        expect(
          AssociationGrandChildResource.association?(:has_one_object)
        ).to be_true

        expect(
          AssociationGrandChildResource.association?(:superclass_has_one)
        ).to be_true
      end

      it 'returns false if the association is on a subclass' do
        expect(
          AssociationResource.association?(:belongs_to_object)
        ).to be_false
      end

      it 'returns false the association does not exist' do
        expect(
          AssociationGrandChildResource.association?(:fake_association)
        ).to be_false
      end

    end

    context '.association_class_name' do

      it 'returns the class string if the association exists' do
        expect(
          AssociationGrandChildResource.association_class_name(
            :has_many_objects
          )
        ).to eql('HasManyObject')
      end

      it 'returns nil if the association does not exist' do
        expect(
          AssociationGrandChildResource.association_class_name(
            :fake_association
          )
    ...
    context '.association_class' do
      ...
      it 'returns the class if the association exists' do
        expect(
          AssociationGrandChildResource.association_class(:has_many_objects)
        ).to eql(HasManyObject)
      end

      it 'raises an error if the class cannot be found' do
        expect {
          AssociationGrandChildResource.association_class(:to_override)
        }.to raise_error(
          ApiResource::AssociationBuilder::AssociationClassNotFound
        )
      end

      it 'returns nil if the association does not exist' do
        expect(
          AssociationGrandChildResource.association_class(:fake_association)
        ).to be_nil
      end
    ...
    context '.association_names' do
      ...
      it 'returns a uniq list of all associations' do
        expect(
          AssociationGrandChildResource.association_names.sort
        ).to eql(
          [
            :belongs_to_object,
            :has_many_objects,
      ...
      it 'does not include associations from descendants' do
        expect(
          AssociationChildResource.association_names.sort
        ).to eql(
          [
            :belongs_to_object,
            :has_many_objects,
  ...
  context 'Using associations on instances' do

    subject { AssociationResource.new }

    # Tested with has_many but works similarly for the rest
    context 'objects' do

      context 'reader' do

        it 'gives back the proper association proxy object' do
          expect(
        ...
        it 'caches the association proxy' do
          expect(
            subject.has_many_objects.__send__(:object_id)
          ).to eql(subject.has_many_objects.__send__(:object_id))
        end

      end

      context 'writer' do
        it 'delegates the assignment to the association proxy' do
          proxy = subject.has_many_objects

          proxy.__send__(:expects, :assign).with([]).returns([])
          subject.has_many_objects = []
        end
      end
    ...
    context 'foreign key' do
      ...
      context 'reader' do
        ...
        it 'delegates reading to the association proxy' do
          proxy = subject.has_many_objects

          proxy.__send__(:expects, :read_foreign_key).returns([1])
          expect(subject.has_many_object_ids).to eql([1])
        end
      ...
      context 'writer' do
        ...
        it 'delegates the assignment to the association proxy' do
          ...
          proxy.__send__(:expects, :write_foreign_key).with([]).returns([])
          subject.has_many_object_ids = []
        end
    ...
    context 'Integration' do
      # This section deals with testing behavior at a higher
      # level for dealing with associations
    end
  ...
  #   before(:each) { pending 'Replace these tests' }
