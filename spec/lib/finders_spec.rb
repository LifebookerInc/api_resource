...
describe ApiResource::Finders do
  ...
  xit "should be able to find a single object" do
    TestResource.connection.expects(:get).with("/test_resources/1.json")

    TestResource.find(1)
  end

  xit "should be able to find with parameters, params syntax" do
    TestResource.connection.expects(:get).with("/test_resources.json?active=true")
    TestResource.all(:params => {:active => true})
  end

  xit "should be able to find with parameters without the params syntax" do
  ...
  xit "should be able to chain find on top of a scope" do
  ...
  xit "should be able to find a single object after a scope" do
  ...
  xit "should be able to find multiple objects after a scope" do
    TestResource.connection.expects(:get)
  ...
  xit "should be able to find the first/last object" do
    TestResource.connection.expects(:get)
  ...
  xit "should pass first and last as params" do
    TestResource.connection.expects(:get)
  ...
  xit "should not pass all as a param, even when all records are requested" do
    TestResource.connection.expects(:get)
  ...
  xit "should be able to chain find on top of an includes call" do
  ...
  xit "should be able to chain find on top of an includes call and a scope" do
  ...
  xit "should disregard condition order" do
    TestResource.connection.expects(:get).with("/test_resources.json?birthday%5Bdate%5D=5&find%5Bids%5D=1").returns({"id" => 1, "has_many_object_ids" => [1,2]})
    HasManyObject.connection.expects(:get).with("/has_many_objects.json?ids%5B%5D=1&ids%5B%5D=2").returns([])
  ...
  xit "should be able to use a scope with arguments" do
    TestResource.connection.expects(:get)
  ...
  xit "should be able to use a scope with multiple arguments" do
    TestResource.connection.expects(:get)
